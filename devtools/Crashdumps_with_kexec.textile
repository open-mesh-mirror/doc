...
Other tools like "crash":https://github.com/crash-utility/crash or "drgn":https://drgn.readthedocs.io/ are better suited for this task.

h3. crash

crash can even show how the page is actually mapped. In our case, the problem is that modules are not using continuous physical pages (which are mapped by the program headers) but only virtual address space continuous pages:
...
drgn is actually a highly programmable debugger - with emphasis on  (but not limited to) retro-active poking in the program state and read-only debugging. Its interactive shell is actually a python interpreter and has to be used like one:
...
But it provides enough helper to get more information about the current state of the system
