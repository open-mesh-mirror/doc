...
.. code-block:: sh
  ...
  mkdir -p machines
  chattr +C machines || true

  for i in $(seq 1 "${NUM_SESSIONS}"); do
      normalized_id="$(echo "$i"|awk '{ printf "%02d\n",$1 }')"
      twodigit_id="$(echo $i|awk '{ printf "%02X", $1 }')"

      qemu-img create -b ../"${BASE_IMG}" -f qcow2 -F "${BASE_IMG_FMT}" machines/root.cow$i
      screen qemu-system-x86_64 -enable-kvm -name "instance${i}" \
          ...
          -drive file=machines/root.cow$i,if=none,id=drive0,cache=unsafe,discard=unmap \
