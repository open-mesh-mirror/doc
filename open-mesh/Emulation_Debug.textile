...
sudo chroot debian /usr/bin/passwd
...
<pre>
screen
./hub.sh # just bridges eth0 with newly created tap devices and runs dhclient on top of bridge
./run.sh # warning, the shared directory has to be changed to a valid one; mounted as /host in client
</pre>