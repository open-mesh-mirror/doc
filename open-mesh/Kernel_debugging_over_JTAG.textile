...
Only the correctly "raw GPIO number for each Pin":https://www.raspberrypi-spy.co.uk/2012/06/simple-guide-to-the-rpi-gpio-header-and-pins/ has to be found. A good way to connect them (while keeping the SPI pins free for reflashing SPI chips) is:
...
The Bus Pirate's SPI pins can also be used for JTAG. But it is an extremely slow debug adapter compared to one which was specifically build for this purpose.
