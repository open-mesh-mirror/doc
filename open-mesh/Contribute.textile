...
** batman-adv patches must also include the target (branch) information as part of the prefix in such a way that netdev/LKML can understand it. <code>[PATCH batadv]</code> for patches targetting the stable branch and <code>[PATCH batadv-next]</code> for the main branch.
...
* Patches against the batman-adv main branch must be formatted using <pre>
git format-patch --subject-prefix 'PATCH batadv-next' $BASECOMMIT
</pre> or an equivalent tool (like <code>b4</code>)
* README and manpage must be updated together with the related source change
* Add or update kerneldoc to functions and structures you add or modify.
* batman-adv changes affecting batctl have to be send with the batctl at the same time
* it is recommended to use "b4":https://b4.docs.kernel.org/ to send the patch mails to the mailinglist - but plain `git send-email` is also fine.
