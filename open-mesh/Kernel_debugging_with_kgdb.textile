...
** set bit n in register GPIO_SET (0x1804000C) to set output value to 1
** set bit n in register GPIO_CLEAR (0x18040010) to set output value to 0
* overwrite complete GPIO_OUT (0x18040008) register (which might modify more GPIO bits then required)

We will only demonstrate this here for GPIO 12 with GPIO_SET/GPIO_CLEAR.

<pre>
# check where 0x18040000 is mapped to
(gdb) display ath79_gpio_base
8: ath79_gpio_base = (void *) 0xb8040000

# set GPIO 12 to low
set {uint32_t}0xb8040010 = 0x00001000

# set GPIO 12 to high
set {uint32_t}0xb804000C = 0x00001000
</pre>