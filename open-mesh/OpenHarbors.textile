...
# A user's internet traffic is typically unencrypted in the WLAN mesh cloud
...
* However, the PMK after it was generated is then forwarded from the RADIUS server to and installed on the AP (ak. authenticator).
...
# Tunnel the full WPA exchange over IP
...
[] create a UDP/L2TP tunnel to parsed domain
...
Option A)
* Setup/Utilize mac80211_hwsim kernel module?

Option B)
* 

  * Remote Server side, initialization / EAP handling:
  * Remote Server side, data forwarding, from client:
    ...
  * Remote Server side, data forwarding, to client:
    ...
...
[] add an openharbor-server package
  [] add documentation/description to package
...
* Kernel module / mac82011 additions for encapsulation for performance - if necessary?
...
* Layer 2 encapsulation method, which skips the IP/UDP/L2TP headers and
