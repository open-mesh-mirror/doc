...
|                                    | *IPv4*          | *IPv6* |
| *all nodes link-scope* |=. x¹ |=. x¹ |
| *link-local*
(excl. all nodes addr.) |=. x¹ |=. x |
| *routable* |=. TODO² |=. TODO² |

x: supported

¹: These addresses cannot be considered for optimization towards nodes which have a bridge interface on top of their batman interface as they are not snoopable. See  [[Basic-multicast-optimizations-flags#BATADV_MCAST_WANT_ALL_UNSNOOPABLES]] for details.

²: Routable multicast addresses are not supported yet. See  [[Basic-multicast-optimizations-tech#routable-multicast-addresses]] for details.

Examples, not optimized:

* 239.1.2.3
* ff0e::101 (global (e), well-known (1st 0) IPv6 multicast address for NTP (101) )

Examples, optimized:

* No bridges, ff12::39 (IPv6 link-local (2) multicast (ff) address, dynamically assigned (1))
* No bridges, 224.0.0.1 (IPv4 all nodes link-scope multicast address)
* No bridges, ff02::1 (IPv6 all nodes link-scope multicast address)
