...
It could also be the case that before the network get in synch again the client roams once more. In this scenario the same procedure described above applies with some minor extensions. In particular this how the node involved in the roaming procedure will react:
* the new node sends a ROAM_ADV to the old node (the very first node serving this client, not the one where the client roamed to the first time)
* the old node updates its global table in order to reroute the packets to the correct (new) destination
* the old node sends a ROAM_ADV to the one which was temporary serving the client to tell it that the roaming phase is over and that the client "came back"

In this way data will be still sent to the old node and then getting rerouted to the correct location with no loss.
