...
It could also be the case that before nodes get in synch again, the clients roams once more. In this case the same procedure described above applies with some minor modifications:
* the new node sends a ROAM_ADV to the old node (the very first node serving this client, not the one where the client roamed to in the first step)
* the old node update its global table in order to reroute the packets to the correct (new) destination
...
In this way the data will continue to flow to the old node first and then getting rerouted to the correct destination.
