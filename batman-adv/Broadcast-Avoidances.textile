...
Currently, we have two problems: For one thing, the computational overhead could be significant for neighborhoods of a certain size. For another, B does not even have the throughput from A to B or A to C (or C to A or C to B - we will see later why we might need those) available. B only knows its own TX throughput towards other neighbors and not the other way around.
...
Instead of checking all (AB < ACn?) and (BC < ACn?) combinations, it tries to perform "one check that rules them all" (with the downside of maybe not catching all potential rebroadcast avoidance cases).

h3. Proofs

h4. Proof: Egress TX (bcast)

We want to proof: Bmax < Amin => BC < ACn

One has:
 * BC ≤ Bmax 
 * Amin ≤ ACn

Bmax < Amin => (BC ≤ Bmax) < Amin => (BC ≤ Bmax) < (Amin ≤ ACn) => BC ≤ Bmax < Amin ≤ ACn => BC < ACn

h3. Proof: Ingress TX (bcast)

We want to proof: Amax ≤ Amin => AB ≤ ACn for any ACn.

One has:
 * AB ≤ Amax
 * Amin ≤ ACn

Amax ≤ Amin => (AB ≤ Amax) ≤ Amin => (AB ≤ Amax) ≤ (Amin ≤ ACn) => AB ≤ Amax ≤ Amin ≤ ACn => AB ≤ ACn
