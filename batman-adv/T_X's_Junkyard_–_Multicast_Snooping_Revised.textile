...
Or convert any IN'(Query)' to an OUT'(MLDv2 Query + MRD-RA)' - nah, still stupid for cases with multiple snooping switches... Or do this conversion and require any query transmitted/forwarded by a snooping switch implemented as specified in this document to set a certain flag in the query? Set the "Code" field of a query to 1 for instance (though this would still leave many "false positives" and Multicast Router Advertisements...)?
...
h3. Notes on Switch-internal Queriersa