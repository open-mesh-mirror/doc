...
These two patches decreased the overall ARP broadcast overhead by 89.86% and the ARP broadcast overhead from gateways by 96.22% in a  ~180 nodes setup at Freifunk Lübeck (firmware v0.19.0+v0.15.5), measured over one week before and after applying the patches on a specific mesh node's mesh-vpn interface.
