...
In larger mesh networks a significant (~30 kbit/s with ~1000 nodes) amount of ARP Requests which are broadcasted by batman-adv can be observed.
...
A) Nearly 90% of all ARP Requests this gateway send were falling back to a batman-adv broadcast.
