...
* LM_MAIN
** name of branch in batman-adv.git which points to the latest patch already ported to batadv.git:batadv/net-next
* LM_STABLE
** name of branch in batman-adv.git which points to the latest patch already ported to batadv.git:batadv/net
* BASE_NETNEXT
...
First step is to actually get the repositories and create the branches. You may also use the --reference option for batadv to speedup the download when you already have some linux repository on you system

<pre>
git clone "${GIT_OM_SERVER}/batadv.git" batadv
git -C batadv remote add net-next https://git.kernel.org/pub/scm/linux/kernel/git/netdev/net-next.git
git -C batadv remote add net https://git.kernel.org/pub/scm/linux/kernel/git/netdev/net.git
git -C batadv remote update --prune
git -C batadv branch base/net --track origin/base/net
git -C batadv branch base/net-next --track origin/base/net-next
git -C batadv branch batadv/net --track origin/batadv/net
git -C batadv branch batadv/net-next --track origin/batadv/net-next
...
The next step is to make sure that out batadv.git branches are either on top or equal the chosen. It should also be checked first that it is really a basepoint in the chosen branches

<pre>
git -C batadv remote update --prune
git -C batadv branch -r --contains="${BASE_NET}" net/main
git -C batadv branch -r --contains="${BASE_NETNEXT}" net-next/main

git -C batadv switch base/net
git -C batadv merge --ff-only "origin/base/net"
git -C batadv merge --ff-only "${BASE_NET}"

git -C batadv switch base/net-next
git -C batadv merge --ff-only "origin/base/net-next"
git -C batadv merge --ff-only "${BASE_NETNEXT}"

git -C batadv push origin base/net base/net-next
</pre>
...
# edit the commit message to point to the correct upstream commit -- just search in batadv for the commit name
git commit --amend
...
First the batadv build environment has to be prepared
...
h2. check if all patches are in batadv
...
<pre>
TAG=batadv-next-for-netdev-20160701
git format-patch -s --cover-letter --subject-prefix='PATCH net-next' base/net-next..${TAG}
sed -i '/\*\*\* BLURB HERE \*\*\*/q' 0000-cover-letter.patch
git request-pull base/net-next https://git.open-mesh.org/batadv.git ${TAG} >> 0000-cover-letter.patch
# write cover letter
git send-email --to=netdev@vger.kernel.org --cc='"David S. Miller" <davem@davemloft.net>' --cc='Eric Dumazet <edumazet@google.com>' --cc='Jakub Kicinski <kuba@kernel.org>' --cc='Paolo Abeni <pabeni@redhat.com>' --cc 'Simon Horman <horms@kernel.org>' --cc=b.a.t.m.a.n@lists.open-mesh.org 00*.patch
</pre>
...
git format-patch -s --cover-letter --subject-prefix='PATCH net' base/net..${TAG}
sed -i '/\*\*\* BLURB HERE \*\*\*/q' 0000-cover-letter.patch
git request-pull base/net https://git.open-mesh.org/batadv.git ${TAG} >> 0000-cover-letter.patch
# write cover letter
git send-email --to=netdev@vger.kernel.org --cc='"David S. Miller" <davem@davemloft.net>' --cc='Eric Dumazet <edumazet@google.com>' --cc='Jakub Kicinski <kuba@kernel.org>' --cc='Paolo Abeni <pabeni@redhat.com>' --cc 'Simon Horman <horms@kernel.org>' --cc=b.a.t.m.a.n@lists.open-mesh.org 00*.patch
</pre>
...
are available in the git repository at:
  ...
  https://git.open-mesh.org/batadv.git tags/batadv-next-for-netdev-20160701
...
Simon Wunderlich (2):
 ...
 net/batman-adv/mesh-interface.c                    |  10 +-
