...
<pre>
$ git clone git+ssh://git@open-mesh.org/linux-merge.git
...
<pre>
(merge/master)$ git remote update
    Fetching origin
    remote: Counting objects: 1387, done.
    remote: Compressing objects: 100% (211/211), done.
    remote: Total 823 (delta 651), reused 762 (delta 602)
    Receiving objects: 100% (823/823), 136.90 KiB | 254 KiB/s, done.
    Resolving deltas: 100% (651/651), completed with 228 local objects.
...
(merge/master)$ git rm -rf CHANGELOG README.external compat.h compat-include/ compat-sources/ gen-compat-autoconf.sh
    ...
    rm 'README.external'
    ...
(merge/master)$ vim net/batman-adv/Makefile
(merge/master)$ git checkout merge/master Makefile README
(merge/master)$ git add net/batman-adv/Makefile
(merge/master)$ git commit
...
<pre>
(merge/master)$ git checkout -b patches
(patches)$ git rebase origin/merge/master
...
(f6d8baba...)$ vim net/batman-adv/Makefile
(f6d8baba...)$ git add net/batman-adv/Makefile
(f6d8baba...)$ git rebase --continue
    ...
    If you would prefer to skip this patch, instead run "git rebase --skip".
    To restore the original branch and stop rebasing run "git rebase --abort".
(f6d8baba...)$ git rm net/batman-adv/compat.h
...
(f6d8baba...)$ git rebase --continue
    ...
    If you would prefer to skip this patch, instead run "git rebase --skip".
    To restore the original branch and stop rebasing run "git rebase --abort".
(a85d1b45...)$ git rm net/batman-adv/Makefile.kbuild
    net/batman-adv/Makefile: needs merge
    net/batman-adv/Makefile.kbuild: needs merge
    rm 'net/batman-adv/Makefile.kbuild'
(a85d1b45...)$ vim net/batman-adv/Makefile
(a85d1b45...)$ git add net/batman-adv/Makefile
(a85d1b45...)$ git rebase --continue
    Already applied: 0009 batman-adv: Add support for git revision string
    All done.
(patches)$ git diff merge/master
</pre>
...
<pre>
(patches)$ git rebase -i origin/merge/master
...
<pre>
(patches)$ git rev-list --reverse origin/merge/master..
...
(166b2caa...)$ make CONFIG_BATMAN_ADV=m CC=cgcc -C ../../ M=`pwd`
...
(166b2caa...)$ git checkout 5cd65234b12021fe1f28e28e455a6b6a06712ec2
...
(83d1bc8b...)$ make CONFIG_BATMAN_ADV=m CC=cgcc -C ../../ M=`pwd`
....
(83d1bc8b...)$ git checkout 9e9605f0fe000f1097cb43271540ab2b76b2a782
...
(9e9605f0...)$ make CONFIG_BATMAN_ADV=m CC=cgcc -C ../../ M=`pwd`
....
(9e9605f0...)$ git checkout 043aa9a54a5296c6027a5bcd808b8d29cea8a474
...
(043aa9a5...)$ make CONFIG_BATMAN_ADV=m CC=cgcc -C ../../ M=`pwd`
...
(043aa9a5...)$ git checkout f6d8baba5fce6161bc2cee9c97e0afbc139942c0
...
(f6d8baba...)$ make CONFIG_BATMAN_ADV=m CC=cgcc -C ../../ M=`pwd`
...
(f6d8baba...)$ git checkout a85d1b45e4cde0c882a41baaefeec0cd5502416f
...
(a85d1b45...)$ make CONFIG_BATMAN_ADV=m CC=cgcc -C ../../ M=`pwd`
...
(a85d1b45...)$ git checkout patches
...
<pre>
(patches)$ git format-patch --histogram -s -M origin/merge/master
...
<pre>
(patches)$ git checkout batman-adv/next
(batman-adv/next)> git rev-parse batman-adv/next
    12513b76a021e5b41a9d5d5981da75dfd6480890
(batman-adv/next)$ git am -3 00*.patch
...
(batman-adv/next)$ git diff merge/master -- net/batman-adv/ Documentation/networking/batman-adv.txt Documentation/ABI/testing/sysfs-class-net-batman-adv Documentation/ABI/testing/sysfs-class-net-mesh
(batman-adv/next)$ git rebase net-next/master
    First, rewinding head to replay your work on top of it...
    Applying: batman-adv: Convert names from Java to C style
    Applying: batman-adv: Avoid rounding issues for local hna timeout
    Applying: batman-adv: Lower resolution for timeouts
    ...
    Applying: batman-adv: adapting source version to revised versioning scheme
    Applying: batman-adv: Add include guards to all header files
(batman-adv/next)$ git push -f origin batman-adv/next
(batman-adv/next)$ git push origin
(batman-adv/next)$ git tag -d batman-adv-for-davem
(batman-adv/next)$ git push origin :batman-adv-for-davem
(batman-adv/next)$ git tag -s batman-adv-for-davem
(batman-adv/next)$ git push origin batman-adv-for-davem
(batman-adv/next)$ git format-patch -s -M --histogram net-next/master..batman-adv-for-davem
(batman-adv/next)$ git request-pull net-next/master git://git.open-mesh.org/linux-merge.git batman-adv-for-davem > shortlog
(batman-adv/next)$ git send-email --compose --to=davem@davemloft.net --cc=netdev@vger.kernel.org --cc=b.a.t.m.a.n@lists.open-mesh.org 00*.patch
</pre>
