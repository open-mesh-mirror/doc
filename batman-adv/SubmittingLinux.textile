...
The git-merge.sh tool used below can be found in the git repo of git project :-) (https://github.com/git/git/blob/master/contrib/subtree/git-subtree.sh)

<pre>
'-(%25)-(xyz)> git checkout merge/master
Checking out files: 100%25 (32337/32337), done.
Switched to branch 'merge/master'
'-(%25)-(merge/master)> ./git-subtree.sh merge --prefix=net/batman-adv pkg/next
...
<pre>
git diff pkg/next: merge/master:net/batman-adv/
</pre>
...
Since git is not able to properly guess the path where the merge has to be done anymore, the rebase command has been slightly changed to use a more advanced form of subtree strategy.
With this new strategy, the path of the subtree can be manually specified.

<pre>
'-(%25)-(merge/master)> git checkout -b patches
'-(%25)-(patches)> git rebase -s recursive -X subtree=net/batman-adv origin/merge/master
...
'-(%25)-(batman-adv/next)> git push origin batman-adv-for-davem
'-(%25)-(batman-adv/next)> git format-patch -s -M --patience net-next/master..batman-adv-for-davem
