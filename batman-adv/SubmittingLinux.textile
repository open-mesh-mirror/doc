...
{{{
'-(%25)-(linux)> git checkout -b next --track origin/next
Branch next set up to track remote branch next from origin.
Switched to a new branch 'next'
'-(%25)-(next)> git pull http://git.open-mesh.org/batman-adv.git next
...
'-(%25)-(linux)> git merge -s subtree next
