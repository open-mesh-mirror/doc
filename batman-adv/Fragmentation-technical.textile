...
* Refragmentation at later hops are avoided by a hard-coded max-fragment-size (1400 bytes).
* Fragments are not checked for tt-updates on the path.
* Size of local translation table is limited to the maximum fragmentation size.
...
* Limit size of local translation table to be less than (min_mtu-frag_hdr-eth_hdr)*16*1400.
   When the mtu decreases, we must clean the local table. Finding the oldest entries is complex, because entries can be updated while searching the list. Instead we traverse the list and delete any entry older than K, half K and traverse the list again, and repeat until the list is shortened enough.
* Clean up this page.
* Write final report.
* Update documentation before Sven reminds me.
* KernelDocument #struct batadv_frag_packet# in packet.h
* Make tests and graphs.
* Document the concept on this page.
* Celebrate!
...
* How do we make locking safe for the merging?
  In the current state, spinlocks are used for insertion and housekeeping of fragments. It is however difficult for me to see, how locking is added in a safe way, when merging packets. Currently, the insert-functions returns the frag_head of the list used for buffering and the caller checks if the list of fragments is complete and merges them if so. Thus there is a unsafe gap between return of the insert-function and the merging of the fragments.
