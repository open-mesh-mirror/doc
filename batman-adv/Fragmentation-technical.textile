...
* Should the contained packet type be visible in the fragment header? No.
* How do we select supported packet types (whitelist, blacklist or some kind of passed flag)? All packets are fragmented (except broadcast and multicast so far).
* Should we make a separate header for the first fragment? No.
** If we make a separate header, we can include assembled size to avoid assembly/fragmentation on intermediate nodes.

h3. Initial header structure:

<pre>
struct frag_packet {
	struct batman_header header;
	uint8_t frag_type; /* Packet type of the contained packet */
	uint16_t frag_id;   /* Almost unique id for the set of fragments (CRC?) */
	uint8_t frag_cnt:4;  /* Number of fragments in this set */
	uint8_t frag_seq:4;  /* Sequence number of this fragment */
	uint8_t flags;
} __packed;
</pre>
