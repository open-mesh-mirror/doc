...
* Use the normal batman-adv broadcast packet but with a MCAST_COMPAT flag set. And otherwise forward like any batman-adv broadcast packet on all hard interfaces.
</code></pre>
...
* Add a new OGM flag: "BATADV_MCAST_OPTIMIZATIONS = BIT(<code></code>7)": Set this flag if multicast optimizations are enabled (e.g. via sysfs) or unset it otherwise.
...
h3. MCAST_COMPAT option mcast_compat_ttl

However when already having a large, non-multicast-optimization-aware network deployed it might is very likely undesireable to do multicast streaming tests even from a node having these multicast optimizations enabled: Still most of the mesh network would simply flood and congest the network as old nodes would keep to the current broadcast forwarding mechanism.

To ease tests in mostly non-multicast-optimization-aware networks (so e.g. for debugging purposes) the addition of a 1 byte MCAST_COMPAT options field in a batman-adv multicast data header is suggested (e.g. instead of the reserved byte), consisting of:

* *mcast_compat_ttl*: The initial, to be set TTL of a batman-adv broadcast packet with MCAST_COMPAT flag made from this multicast data packet. Setting it to 0 would disable sending an optimized multicast stream to any non-multicast-optimization-aware node (hence disabling any broadcast flooding of such packets).
