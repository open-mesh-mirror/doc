...
MCAs are MAC address based. Those addresses are currently distributed via our periodic OGMs.
...
Multicast Path Tracking combines the MCA and flow infrastructures: On sufficient multicast data flow for a specific multicast destination MAC, small tracker packets are actively sent to mark all paths towards destinations, destinations which were previously announced via MCAs.
...
A batman-adv node broadcasting its own OGM, an originator, has to attach a 6 byte multicast MAC address for any of its multicast listeners after the OGMs TT diff part.
...
The body of a multicast tracker packet needs to be filled with a Multicast Tracker Packet Entry for any multicast MAC address which is present in the MCA buffer of other originators and which has a matching multicast flow state which is "high".
...
This generated Multicast Tracker Packet then gets scheduled for forwarding (see "Tracker Packet Forwarding").

h4. Reactive Tracker Packet Transmission

A Tracker Packet might further get generated if a multicast flow threshold state switched from "low" to "high" (see "Multicast Flow Measurement").

Such a tracker packet gets generated similar to the periodic one but for the specific multicast MAC address which triggered the state switch
only. Which means that the reactively generated tracker packet will have a "Num Mcast Entr." set to 1 and only one Multicast Tracker Packet Entry.
