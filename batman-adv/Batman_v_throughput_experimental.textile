...
** Unicast message for link thgoughput exchange (one per neighbour). [One broadcast can be used on wired interface]
* OGMv2
...
No need to count sequence numbers anymore
...
** one OGM is sent as unicast to N containing the link metric value (namely OGM.metric) equal to 0
* A generic node P receiving an OGM from R
** computes next_hop_metric = nhm(OGM.metric, R)
where nhm(x, r) is a metric alterating function which computes the path metric towards OGM.SRC by combining OGM.metric and the estimated throughput on the link P <-> R.
** For each interface I
*** computes forw_metric = fwm(next_hop_metric, I)
where fwm(x, i) is a metric alterating function which computes the metric value that a node has to propagate over the given interface i
