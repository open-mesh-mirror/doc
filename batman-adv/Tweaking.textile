...
The [wiki:batman-adv-gateways internet gateway support] allows each gateway to also announce its available internet bandwidth. Clients looking for the most suitable gateway to connect to receive this bandwidth announcement and can make use of it while choosing their gateway. Per default a bandwidth of 2048KBit/512KBit is assumed. Details regarding the syntax of the bandwidth setting can be found in the [http://downloads.open-mesh.org/batman/development/sources/batctl_devel-current_sources/man/batctl.8.html batctl manpage].
