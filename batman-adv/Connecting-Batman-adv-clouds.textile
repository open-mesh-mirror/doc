...
h3. New GW Advertisement

A first consequence of this mechanism is that each border gateway which is able to route packets towards a given subnet, say S1, has to create a client configured with a MAC address which has to be the same on all the gws. Such MAC address can be computed using a function so that, given S1, Func(S1) gives the same result on all the nodes. It is also important that, given two subnets S1 and S2, Func(S1) is different from Func(S2), therefore Func must be built on top of a robust hash.