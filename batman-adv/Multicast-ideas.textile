...
In the beginning when an optimised distribution infrastructure has just started to build up, multicast data packets should still be broadcasted. The advantage of this would be, that no buffering or packet dropping would have to be done.
 ...
 * Is this a multicast packet which shall take an optimised path (bcast_packet.packet_type == BAT_MCAST)?
