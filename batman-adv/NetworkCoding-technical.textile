...
The header is can be logically divided into two main parts: One part for each of the combined packets. For each packet, the header specifies the next-hop (although the "first next hop" is given in the MAC header), the previous source, the final originator destination and a CRC32 checksum of the packet.

To identify the packet needed to decode the network coded packet, the receiver must reconstruct a (src,dst,crc) tuple from the received network coded packet. The first field (src) is the original source, which is given as either first_source of second_source in the header. The second field (dst) is the source of the relay, which is found in the MAC header. The third field (crc) is found as either first_crc or second_crc in the header of the network coded packet.
...
The field coded_len is needed when the packets of different length is network coded, as only a part of the resulting packet will actually be coded. The coded_len field tells the receiver the length of the shortest combined packet, as the receiver of this only knows the length of the longer packet.
