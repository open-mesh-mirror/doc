...
A new bat-ICMP type must be defined for the bandwidth meter packets: two headers are attached to the "data" the bat-ICMP and a pseudo-TCP one (shown in next paragraph). 

_How a node can ask to another node to be the sender?_
# Through an option field in the pseudo-TCP header in the three way handshake process.
# Different bat-ICMP numbers

_How does the protocol end?_
# graceful close with FIN.
## If the node that has initiated the protocol is the sender, after all the data is sent, send a packet with FIN flag. A packet with FIN flag should be received in response from the other node, meaning that all data has been delivered, thus the test is over;
## If the node is the receiver, when a packet with FIN flag is received, it waits for missing packets (if any) and then close the test.
## TODO: check how TCP works in case FIN is lost or in case after FIN is received packets are still missing.

h3. Header

Since the protocol is taken from TCP, I started from TCP header ("RFC793":http://www.ietf.org/rfc/rfc793.txt) and removed some unecessary features, namely push, urgent and port multiplexing.
...
* define user-space interface (through batctl)

* being a connection oriented protocol, some data structures relative to each connection will be defined. Check TCP implementation of such data structures and how timeouts are used to handle connections.

* try virtual machines for testbed
