...
During all the test ICMP packets only are exchanged: to distinguish the bandwidth meter packets from the others, two fresh ICMP packet types are defined in packet.h: 
<pre>
...
# if packet offset < first_received, resend ack *with sequence number = first_received*
