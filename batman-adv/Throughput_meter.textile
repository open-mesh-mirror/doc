...
** initialize data structure on receiver side

** account for the received size

* react to the ack reception

* can use __be16 seq of icmp_packet instead of a uint32_t offset ?? Are 16 bit of sequence number enought? 1500*2^16=93MB (seq can also wraparound??)

h3. Future tasks

* check bw_meter_start concurrent access to bat_priv

* create a list for bw_meter_vars or store it in origin_node? [anyway currently there is just one for everybody, so only one test at a time. To be fixed]

* do the receiver ???

* sometimes packets are not received

* check for the list lock
...
* define the BW_METER icmp packet header in packet.h <pre>
...
The protocol is connection oriented. Of the two end nodes, one is the sender and the other the receiver. Sender and receiver behaviour is described in the next sections.

During all the test only ICMP packets are exchanged: to distinguish the bandwidth meter icmp packet from the others, two fresh ICMP packet type are defined: 
<pre>
...
In order to provide an easier protocol to implement, Go-Back-N is used, with a static window. The window is made of WIN_PACKETS packets (this could be specified by the user or fixed).
...
It sends WIN_PACKETS packets in a burst and starts a timeout, then waits for an ack, that is an ICMP packet with number BW_ACK. The ACK will carry the sequence number of the last received packet *in order* (cumulative acknowledgment), meaning that all the packets up to that sequence number have been received. If the ack for a packet is not received before the timeout expires, all the packet unacked in the window are re-transmitted. Notice that there is no buffer, just the three variables that "emulates" it. The sender ICMP packet has type BW_METER, a 32 bit packet number and a 32 bit window size.

So WIN_PACKETS are sent, then the a timeout is started:

* when the timeout expires, the whole window is transmitted again;
* if an ack is received, flush the timer. Update first_sent to ack's "sequence number" and send (first_sent - previous_first_sent) new packets. Start the timer with the default timer value;

h3. The receiver

To begin the test, the receiver must receive the packet with seq 0 first: if another packet is received, the test is not considered started, no ack are sent back and no data structure is allocated on receiver side. 

If packet number zero is received and no other bandwidth test is ongoing with the same node, the following variables for that connection are initiated:
...
# if it is first_received <= offset < first_received+window_size, the packet belongs to the current window. 
## If packet's sequence number  = first_received + 1: first_received+=1; send an ACK; (if we keep track of unordered received packets, can be incremented also of more than 1)
...
# if packet offset > first_received+window_size ??? SOMETHING IS WRONG....send an acK?
...
So, the ACK packet is an ICMP packet with type BW_ACK and a 32 bit sequence number (or __be seq of icmp_packet)?.
