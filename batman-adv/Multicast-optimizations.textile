...
h3. A) No multicast listener/router: Discard
...
h3. B) batman-adv multicast packet

!{width: 66%;}basic-multicast-multiple-receivers-mcast.png!

If all nodes support the [[Multicast-Packet-Type|batman-adv multicast packet type]] and encapsulating the IP packet in it, with all its destination node entries included, would fit into a 1280 bytes frame (excluding outer ethernet frame header) then this packet type is chosen for forwarding.

h3. C) batman-adv unicast packet(s)
...
By default multicast optimizations are activated. You can force the old behaviour, [[Broadcast|classic flooding]] of all multicast frames, on a certain node via the following command, executed as root (here: bat0):
