...
# DAT has been designed to work with  the *Address Resolution Protocol* (ARP) only. Such protocol was made to resolve IPv4 into MAC Addresses and therefore *DAT cannot support IPv6*.
# DAT cannot distinguish between nodes supporting this new feature and nodes that do not. Therefore it is highly recommended to install DAT on all the nodes in your network. If you don't do so the network will still work (see Fallback mechanism for details) but it will not fully exploit the DAT potential.