...
The B.A.T.M.A.N. protocol originally only used a single message type (called OGM) to determine the link qualities to the direct neighbors and spreading these link qualities information through the whole mesh. This procedure is summarized on the [wiki:BATMANConcept B.A.T.M.A.N. concept page] and explained in details in [http://tools.ietf.org/html/draft-wunderlich-openmesh-manet-routing-00 the RFC draft] published in 2008.
...
Each node periodically (NDP interval) generates and broadcast NDP messages for each interface B.A.T.M.A.N. is running on. A jitter may be applied to avoid collisions. The NDP message must be initialized as follows:

 * Packet type: Initialize this field with the NDP packet type.
 * Version: Set your internal compatibility version.
 * Originator Address: Set this field to the MAC address of this B.A.T.M.A.N. interface.
 ...
 * Number of neighbors: The number of neighbors that are announced with this message.

If this B.A.T.M.A.N. interface wants to announce neighboring nodes it should append a neighbor message for each neighbor to be announced and fill the "number of neighbors" field accordingly.

The Neighborhood Discovery Protocol (NDP) Format:
{{{
 0                   1                   2                   3
 ...
 * why orig field if we have sender address ??
 * If the sender address of the NDP belongs to one of the B.A.T.M.A.N. interfaces the message must be silently dropped as this NDP message originated from this node.
 * what happens if the NDP message from interface1 is received on interface2 ???
 * If the NDP message is a duplicate the message must be silently dropped.
 * out of order to be dropped ?
