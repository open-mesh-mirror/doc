...
Once the test is finished, batman-adv will send back a particular packet, namely the *ICMP BW RESULT*, containing the _time spent for the test_ and the _amount of bytes correctly acked by the receiver_. This information is then used by batctl to properly show the result to the user.