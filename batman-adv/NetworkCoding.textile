...
Another support scenario in which network coding can help to save air time is the X-topology, where two sets of nodes communicate through the same relay:
...
Since relays keep back packets to wait for opportunities to network code packets, it introduces a delay of up to 10ms at each hop in a network. If the load increases enough, more opportunities to network code should appear, and delay can actually be decreased by combining packets.

For TCP traffic, the effect for two intersecting TCP flows is limited, as seen in the figure:

!tcp.png!

This is due to the extra delay and jitter that do not interact well with the congestion avoidance algorithms of TCP. It is assumed that one would see a gain, if more than just two flows congest the links. 
