...
h3. 3.1 Broadcasting own Originator Message 2 (OGM2)
 ...
 * *Age check:* If the sequence number is strictly older than the last OGM2, the packet is silently dropped. The only exception is when the protection window has just been activated, then the OGM2 can pass.
 ...
 * If the OGM2 sequence number is not newer or equal compared to the last received OGM2 (from any neighbor for this originator) and if the throughput is not better, drop the OGM2
...
h3. 4. Re-broadcasting other nodes' OGM2s
