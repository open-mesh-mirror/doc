...
In its current structure the batman-adv Originator packet is rather static and contains fields which transport feature-specific information. As a result every change to a feature message or every new feature breaks the protocol and needs an increase in the compat_version number.
The project's goal is to implement  a framework to use and transport  IEs (information elements) and more specifically TLVs (type/length/value) elements.
...
<pre>
 ...
 +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 |TVLV Type(0x01)|    Version    |    Length     |  Value
 +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
</pre>
...
# The API decides transparently wether to register and send a long or short tvlv based on the length.
# In order to update a tvlv all you have to do is call the register function again with  new values for version,length,value

h3. Call a handler without a received tvlv

There is a special field for the handlers named +call_always+, it enables the handler to be called each time an OGM was received with a value of NULL  and a 0 as version even if the appropriate tvlv wasn't received.
Its useful for features which used to await a NULL in the ogm to turn off a feature or unregister something.
