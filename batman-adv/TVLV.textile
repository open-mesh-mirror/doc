...
TODO
   ...
   * apped all existing tvlvs from the list to the ogm  --done
    * parameter alignment             --done(?)
   * function names                       --done(?)
   * group re-occuring tasks in separate functions (like tvlv_find()) --done(?)
   *git rebase master -i                
...
#output of the make command on the commit messaged: : http://pastebin.com/iRGWgtmn
...
h2. Questions: 
1)The find_unused symbols prints all of my function names but how can i know which symbol is the unused one?
2)How can i split struct batadv_tvlv_handler_node *tvlv_get_handler(struct batadv_priv *bat_priv, uint8_t type) ?
3)Is there somewhere the description of the function names? (find_wrong_namespace.sh returns all of my functions)
4)i cloned the git://git.open-mesh.org/build_test.git but it didnt bring the sparse, cppcheck folders its 6 am  and i'm probably doing something wrong but please enlighten me



h2. Reserved for arbitrary comments, communication with mentors
	...
	*No more warnings						-- done
  	...
  	*Tvlv handler list 						--done
 	*The Gateway callback should call batadv_gw_node_update()	--done
 	*Remove it from bat_iv_ogm.c and use it in the gateway callback	--done p.s. i'm an idiot
	*Add this to sysfs.c -> batadv_store_gw_mode() 			--done
	...
	case BATADV_GW_MODE_CLIENT:
		/*unregister gw tvlv*
		tvlv_unregister(BATADV_GW_TVLV,
				bat_priv);
		break;
	case BATADV_GW_MODE_SERVER:
		/*register,value == BATADV_GW_MODE_SERVER*
		tvlv_register(BATADV_GW_TVLV, 0x01,
				sizeof(bat_priv->gw_bandwidth),
				&bat_priv->gw_bandwidth, bat_priv);
		break;
	}

   *Fix identation                                                                                --done
   *Reduce easily reducable patches

   *Replace memcpy((char *)tvlv+sizeof(struct shortTvlv),		--done
    		      value, length*sizeof(uint8_t));
	with  memcpy(tvlv + 1, value, length*sizeof(uint8_t));			
	or memcpy(tvlv + 1, value, length);				not applicable sizeof(uint8_t) is there as a reminder that we should care about longTvlvs in the future

	*batctl gwl							
		batadv_gw_node_update() ->  batctl gwl
		disable aggregation
		batctl ag 0
       *fix merge conflicts						--done(?)
 
       *make bat_priv first argument					--done

	* there is a bug, when I sent tvlvs the tvlv process is called for some reason even if the other node does not sent anything

 *   444:/*just register the gateway handler for now
 *  and call the function in the general handler init*/
*   594:/*use batadv_tt_append_diff for reference
batadv_tt_changes_fill_buff*/
