...
Its rather simple to change a feature to use tvlvs. All that is needed to be done is a call to 
<pre>
...
in order to first register a tvlv. You can call the same function in order to update the value field,
The value field can contain whatever is needed by the feature and was transported otherwise till now.
...
<pre>void batadv_tvlv_handler_unregister(uint8_t type,
...
# A long tvlv's type must be bigger than 0x81 otherwise only the first 255 bytes will be considered for the value field

h3. Handlers

There is a specieal field for the handlers named call_always this enables the handler to be called with a NULL as value even if the appropriate tvlv wasnt received.
Its useful for features which used to await a BATADV_NO_FLAGS in the ogm.
...
The purpose of this code is to register and unregister a tvlv when batctl gw server and batctl gw client/off are runn respectively.
...
After that hopefully tvlv will be part of the next compatibility bump.
