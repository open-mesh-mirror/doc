...
In batman-adv we will be using two kinds of tvlvs 
A short tvlv is defined bellow
<pre>
 0                   1                   2                   3
...
And a long one is defined bellow
...
Its rather simple to change a feature to use tvlvs. All that is needed to be done is add a call to 
<pre>
...
in order to first register and start appending a tvlv to each ogm.
You can call the same function in order to update the value field,

Fields: 

* *uint8_t type*: The tvlv's type should be defined as a field in enum tvlvs in packet.h
...
* *uint8_t version*: Thats entirely up to you and its completely transparent to the tvlv infrastructure, only operations on it is copy to the packet and read from packet- pass to handler.
* *uint16_t length*: The length of the value field, depending on the type field the size that will be copied on the packet is either one or two bytes.
* *uint8_t *value:* A field of length "length" that contains whatever information you want to be transfered.
...
Fields: 
uint8_t type: The type of the tvlv you want to stop transporting.

Two restrictions apply:
# If the tvlv shouldnt be ignored there should be the appropriate handler which is registered and unregistered on ocassion by
...
* Every Callback function should return nothing and accept three arguments 
a pointer to struct batadv_priv
a pointer to struct batadv orig_node
a pointer to a value field
and a version byte
<pre>
 void callback(struct batadv_priv *bat_priv, struct batadv_orig_node *orig, uint8_t *value, uint8_t version);
</pre>

h3. Handlers

There is a specieal field for the handlers named call_always this enables the handler to be called with a NULL as value and a 0 as version even if the appropriate tvlv wasnt received.
Its useful for features which used to await a BATADV_NO_FLAGS in the ogm to turn off a feature or unregister something.

h2. Example

Bellow is an example using the easiest feature to be tvlved, the Gateway Feature.
In order to register a new tvlv the following was added to sysfs.c:batadv_store_gw_mode()

<pre>
switch (gw_mode_tmp) {
	case BATADV_GW_MODE_OFF:
	case BATADV_GW_MODE_CLIENT:
		batadv_tvlv_unregister(bat_priv, BATADV_GW_TVLV);
		break;
	case BATADV_GW_MODE_SERVER:
		batadv_tvlv_register(bat_priv, BATADV_GW_TVLV, 1, 1,
				     (uint8_t *)&bat_priv->gw_bandwidth);
		break;
	}
</pre>
The purpose of this code is to register and unregister a tvlv when batctl gw server and batctl gw client/off are run respectively.

In order to update the tvlv when the gateway bandwidth changes we needed the following in gateway_common.c:batadv_gw_bandwidth_set()
<pre>
if (atomic_read(&bat_priv->gw_mode) == BATADV_GW_MODE_SERVER) {
		batadv_tvlv_register(bat_priv, BATADV_GW_TVLV, 1, 1,
				     (uint8_t *)&gw_bandwidth_tmp);
</pre>

While the callback function is registered in main.c:batadv_tvlv_handler_init()
<pre>
batadv_tvlv_handler_register(bat_priv, batadv_gw_tvlv_callback,
		                     BATADV_GW_TVLV, 1);
</pre>
The batadv_gw_tvlv_callback function is defined in gateway_client.c as follows
<pre>
void batadv_gw_tvlv_callback(struct batadv_priv *bat_priv,
			     struct batadv_orig_node *orig_node,
			     uint8_t *value)
{
	uint8_t gw_flags = *value;

	if (!value)
		gw_flags = BATADV_NO_FLAGS;

	if (orig_node->gw_flags != gw_flags)
		batadv_gw_node_update(bat_priv, orig_node, gw_flags);

	orig_node->gw_flags = gw_flags;
...
The following code part was moved in callback from bat_iv_ogm.c
<pre>
/* restart gateway selection if fast or late switching was enabled */
	if ((orig_node->gw_flags) &&
	    (atomic_read(&bat_priv->gw_mode) == BATADV_GW_MODE_CLIENT) &&
...
+NOTE+ : Notice that the gateway callback does not care about version at least for now.
...
Having a (somewhat acceptable) infrastructure and after the GSoC mentors finish reviewing it and finish proposing changes, the patches will be sent to the mailing list for public review.
After that hopefully tvlv will be part of the next compatibility bump along with the first three tvlv'ed features.
