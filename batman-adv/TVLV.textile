...
TODO
   ...
   * Building an emulated testing environment and familiriasing myself with the tools needed. -- Done (somewhat, stays on the currently doing list since its a constant work in progress)
    * first user: gateway settings
   * apped all existing tvlvs from the list to the ogm
    * parameter alignment
   * function names
   * group re-occuring tasks in separate functions (like tvlv_find())
   *git rebase master -i
    *  tvlv handler list
   * register/unregister gateway handler
  * parse incoming tvlv
