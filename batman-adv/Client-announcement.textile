...
* Too many local clients: The size of the local translation table depends on the number served clients. This size cannot exceed the maximum packet size and if the limit is reached, new clients are ignored.
* Decreased MTU: If an interface with and MTU less than the currently activated interfaces is added, the size of the local table might need to be reduced. Otherwise the node is unable to answer requests for full tables.
