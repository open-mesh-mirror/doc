...
The new entry is added to the global TranslationTable and marked as _temporary_ with a special flag (BATADV_TT_CLIENT_TEMP) until a node claims it with the classic announcement mechanism. If none of the mesh node belonging to the mesh network will announce the temporary client, the latter will be deleted upon a timeout expiration set for the purpose.

h2. Limitations

* Too many local clients: If the number of local clients that connects to a node within one originator interval exceeds the limit of entries that fit into a single packet (1500 bytes) the exceeding clients won't be announced. This problem will be solved by the new fragmentation mechanism (not yet implemented) that will enable TT_Response to be fragmented.
