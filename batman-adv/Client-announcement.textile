...
access the mesh network. In case of Layer3 protocols, clients simply are IP
addresses, while in this case, as you can guess, clients consists in MAC
addresses.
...
clients (mesh interface's MAC address too).

h2. The local translation table

Every client MAC addresse ecognised through the mesh interface will be stored
...
the table: each time the node decide to spream the table around, if something
...
achive this, another table is needed: the "global translation table". It is a
...
At boot time, every node will have an empty local table and empty gloal one.
...
This distintion is done using the TT_FULL_TABLE bit of the bitwise _flag_
field in the tt_query packet.

The originator that receives the TT_REQUEST message will reply with a
TT_REQUEST response at the of which it will append the requested data.

The TT_REQUEST message will contain the following fields:
In particular the TT_REQUEST/REPONSE messages are two subtypes of the TT_QUERY
