...
_Note: This page documents internal stuff of batman-adv. Part of it is not yet merged in the batman-adv but is part of the bridge loop avoidance II extension, so read with care_

h2. Flags overview

This section describes the flags used in the TT code, what they mean and where they are used (OGM, tt_global or tt_local).

h3. TT_CLIENT_DEL

_used in OGM, tt_local_
...
It is also set with the TT_CLIENT_PENDING flag to mark a tt_local entry for deletion internally.
...
The private TT_CLIENT_PENDING flag is set for local entries which should be removed. These entries are not removed immediately, but after a timeout to keep the information sent with the OGM or tt_requests consistent.
