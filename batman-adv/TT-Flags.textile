...
| TT_CLIENT_ISOLA | OGM, tt_local, tt_global | The TT_CLIENT_WIFI is used to announce that packets sent by this client were marked with the isolation mark. A node receiving packets from this client has to mark the related skbs with the configured isolation mark (this behaviour is part of the [[Extended-isolation|extended isolation]]). |
