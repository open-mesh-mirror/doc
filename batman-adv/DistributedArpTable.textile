...
DAT will not use any broadcast packet. Node A gets the ARP requests, computes the group of nodes related to the requested IP (in the following picture we assume only node E to be in this set) and sends the ARP request as unicast (green arrows). Node E will reply to node A (blue arrows), the latter will first forward the packet to the non-mesh client that originated the ARP request and then it will store the received entry in its local storage for further caching.
