...
The easiest communication is the push of data to an server. The client just has to create a alfred_push_data packet with the attached data at the end. The server receives it, stores it, closes the client connection and handles the forwarding of the data to the other servers in the alfred network.
...
The last packet is an alfred_status end packet with the number of sent alfred_push_data packets during this synchronization transaction. If it doesn't match with the number of packets the master server received then the transaction failed and the remote master server drops the received data.
